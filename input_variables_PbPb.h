...
TString colliding_system = "PbPb"; // use one of this options = "pp", "pPb", "XeXe" and "PbPb" (OO and pO in the future)
int sNN_energy_GeV = 5020;		   // center of mass colliding energy (GeV)
int year_of_datataking = 2018;

bool do_CM_pPb = false; // do center-of-mass correction in pPb?
bool is_pgoing = false; // is p-going direction?

bool use_centrality = true; // only true for: "XeXe" and "PbPb" (but also can be set as false to see evolution with multiplicity)

float vz_cut_min = -15.0; // vz acceptance
float vz_cut_max = 15.0;  // vz acceptance

const std::vector<double> multiplicity_centrality_bins{10.0, 60.0, 120.0, 185.0, 250.0, 400.0, 3500.0}; // multiplicity range
// const std::vector<double> multiplicity_centrality_bins{0.0, 10.0, 20.0, 30.0, 40., 50., 60., 70., 80., 90., 100., 110., 120., 130., 140., 150., 160., 170., 180.}; // multiplicity range
// const std::vector<double> ntrkoff_bins{10.0, 185.0, 250.0, 500.0, 3000.0};										// multiplicity range
...
std::vector<TString> event_filter_str{"pprimaryVertexFilter", "pphfCoincFilter2Th4", "pclusterCompatibilityFilter"}; // event filters to be applied (PbPb - 2018)
...
float jet_eta_max_cut = 4.0;  // jet min eta cut

bool do_jet_smearing = false; // smearing

TString JEC_file = "Autumn18_HI_V8_MC_L2Relative_AK4PF.txt";	 // JEC file
TString JEU_file = "Autumn18_HI_V8_MC_L2Relative_AK4PF.txt";	 // JEU file (future)
...
float leading_pT_max = 81600.0;									 // used for jet leading and subleading correlation and jet quenching analysis
...
const std::vector<double> trk_pt_bins{0.5, 1.0, 2.0, 3.0, 4.0, 8.0, 12.0, 300.0}; // trk pT bin range for correlations
...
float pthatmin = 10.;			// pT hat cut min
float pthatmax = 900.;			// pT hat cut max
...
void print_input(TString data_or_mc, TFile *fileeff, TString coll_system)
{
	...
	cout << "Event filters applied: {";
	for (int a = 0; a < event_filter_str.size(); a++)
	{
		if (a == event_filter_str.size() - 1)
		{
			cout << "" << event_filter_str[a] << "}";
		}
		else
		{
			cout << "" << event_filter_str[a] << ",";
	...
	if (!use_centrality)
		cout << "Multiplicity bins: {";
	for (int a = 0; a < multiplicity_centrality_bins.size(); a++)
	{
		if (a == multiplicity_centrality_bins.size() - 1)
		{
			cout << "" << multiplicity_centrality_bins[a] << "}";
		}
		else
		{
			cout << "" << multiplicity_centrality_bins[a] << ",";
	...
	cout << endl;
	if (do_CM_pPb)
		cout << "Center-of-mass correction for side: " << endl;
	cout << endl;
	cout << "=========== Jets ===========" << endl;
	cout << endl;
	...
	if (use_WTA)
	{
		cout << "Using WTA jet axis" << endl;
	}
	else
	{
		cout << "Using E-Scheme jet axis" << endl;
	}
	...
	if (coll_system == "PbPb" || coll_system == "XeXe")
	{
		...
		if (coll_system == "PbPb" && sNN_energy_GeV == 5020 && year_of_datataking == 2018)
			cout << "For reco track algorithm 6 remove MVA values less than 0.98" << endl;
	}
	cout << "Track pt bins for correlations: {";
	for (int a = 0; a < trk_pt_bins.size(); a++)
	{
		if (a == trk_pt_bins.size() - 1)
		{
			cout << "" << trk_pt_bins[a] << "} GeV";
		}
		else
		{
			cout << "" << trk_pt_bins[a] << ",";
	...
	if (!fileeff->IsOpen())
	{
		cout << "Cannot find the track efficiency file. Force quit!" << endl;
		return;
	}
	else
	{
		cout << "Track/particle efficiency file: " << trk_eff_file.Data() << endl;
	}
	cout << endl;
	...
	if (do_mixing)
	{
		...
		if (similar_events)
			cout << "Using similar events!" << endl;
	}
	if (do_rotation)
	{
		cout << endl;
	...
	if (do_inclusejettrack_correlation)
		cout << "+ --> Inclusive Jets + tracks (or particle) correlations       +" << endl;
	if (do_leading_subleading_jettrack_correlation)
		cout << "+ --> Leading Jets + tracks (or particle) correlations         +" << endl;
	if (do_leading_subleading_jettrack_correlation)
		cout << "+ --> Sub-Leading Jets + tracks (or particle) correlations     +" << endl;
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
}
